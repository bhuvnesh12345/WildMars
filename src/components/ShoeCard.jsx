...
const ShoeCard = () => {
  ...
  const addHandle=()=>{
    const r=addToCart(shoesData[id-1])
    setMessage(r);
    setTimeout(()=>{
        setMessage("")
 ...
 {/* Show message when item exists or is added */}
 <div className="h-12">
 {message && (
                <p className={`mt-2 text-sm ${message.includes("already") ? "text-red-600" : "text-green-600"}`}>
                    {message}
                </p>
            )}
 </div>
